...
const invertedIndex = JSON.parse(
  fs.readFileSync("src/data/invertedIndex.json")
);
const dataIndex = JSON.parse(
  fs.readFileSync("src/data/data.json").toString("utf8")
);
...
const getProductCandidateIndices = (compounds, lang) => {
  ...
  const maxCount = entries
    .map(x => x.count)
    .reduce((x, y) => Math.max(x, y), 0);
  return {
    maxHit: maxCount,
    entries: entries
      .filter(index => index.count === maxCount)
  ...
  return candidates.entries
    ...
    .map(entry =>
      Object.assign({}, entry, {
        ...
        score:
          entry.tags[query] ||
          documentCompoundScore(compounds, entry.tokenScores),
...
const queryProducts = queryData => {
  const candidates = getProductCandidateIndices(
    queryData.compounds,
    queryData.lang
  );
  const products = assignAndScoreCandidates(
    Object.assign({}, queryData, { candidates: candidates })
  );
