...
const SgGrpc = grpc.loadPackageDefinition(
  protoLoader.loadSync("./src/sg.proto", {
    keepCase: false,
    longs: String,
    enums: String,
    defaults: true,
...
const processError = websocket => error => {
  if (websocket.readyState === 1) {
    ...
    websocket.send(
      JSON.stringify({
        event: "error",
        data: { error: error.message, code: "G01" }
      })
    );
...
const createProductQuery = (tokens, entitySpans, lang, maxProducts) => {
  const getSpanIndex = name =>
    entitySpans.map(span => span.label).indexOf(name);
  const getSpan = index =>
    index >= 0 ? entitySpans[index] : { start: 0, end: 0 };
...
const numOfNonEntityTokens = tokens => {
  return tokens.reduce(
    (count, token) =>
      token.positionInEntity === "outsideOf" ? count + 1 : count,
    0
  );
};
...
const getEntitySpans = tokens => {
  return getSpansByStart(
    tokens,
    token => token.positionInEntity === "startOf"
  ).map(span => {
    return {
...
const findSegments = utterance => {
  return getSpansByStart(utterance.tokens, token => token.isSegmentStart).map(
    span => utterance.tokens.slice(span.start, span.end)
  );
};
...
const findSegmentsWithProducts = (utterance, language) => {
  try {
    ...
    const queriesForSegments = segments.map(tokens =>
      createProductQuery(tokens, getEntitySpans(tokens), language, 5)
    );
...
const parseProductSegments = utteranceData => {
  ...
  const confidences = softMax(
    utteranceData.alternatives.map(alternative => alternative.confidence)
  );
  const alternativeSegments = utteranceData.alternatives.map(alternative =>
    findSegmentsWithProducts(alternative, language)
  );
  const scores = softMax(
    alternativeSegments.map(alt =>
      alt.map(seg => seg.score).reduce((x, y) => x + y, 0.0)
    )
  );
  ...
  const bestSegments =
    alternativeSegments.length > 0
      ? alternativeSegments
          .map((segments, i) => ({
            segments: segments,
            score: Math.log(confidences[i]) + alpha * Math.log(scores[i])
          }))
          .reduce((best, current, i) =>
            i === 0 ? current : current.score > best.score ? current : best
          ).segments
      : [];
        ...
        try {
          websocket.send(
            JSON.stringify({
              event: "transcription",
              data: {
                utteranceId: data.utterance.utteranceId,
                type: data.utterance.type,
...
const handler = (ws, token, params) => {
  const sampleRateHertz = params.sampleRate
    ...
    if (typeof message === "string") {
      ...
      if (event.event === "start") {
        ...
        return ws.send(
          JSON.stringify({
            event: "error",
            data: { code: "C01", details: "Unknown event from the client" }
          })
        );
      }
    } else {
